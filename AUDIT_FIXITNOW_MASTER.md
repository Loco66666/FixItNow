...
- **PHASE 05 (engagée)** : actions pro (accept/en-route/arrive/diagnose/complete), tracking temps réel, `ProviderRealtimeStatus`, décision storage/realtime.
  - **`5.1 POST /interventions/:id/match/:candidateId/accept` — FAIT ✅** : lock atomique `findOneAndUpdate` (candidat PENDING→ACCEPTED) + cascade intervention `REQUESTED|SEARCHING → ACCEPTED` (verrou `professional: $exists:false`, single-row compare-&-swap, zéro transaction Mongo nécessaire) ; classification précise `404/403/409` ; rollback `DECLINED` si la cascade est violée par une course concurrente ; `InterventionStatusHistory` (fromStatus→toStatus, actor=user) ; middleware `requireProfessional` (domainRole PRO) + rate limit `interventions.match.accept` (30/min) + validation Zod partagée `interventionMatchCandidateParamSchema` ; champ `acceptedAt` indexé sur `MatchingCandidate` + type partagé. 5 tests d’intégration (happy / 409 race / 403 stranger / 403 customer / idempotence 409) → **100/100 tests, typecheck + lint + 12 suites verts.**
