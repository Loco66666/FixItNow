...
- **PHASE 07 (engagée)** : Stripe — pré-autorisation + capture à COMPLETED, commission, webhooks idempotents.
  - **`7.0 Paiements Stripe (pré-autorisation/capture/commission) — FAIT ✅`** : gateway **injectable** `services/stripe.client.ts` (interface `StripeGateway` createManualCaptureIntent/captureIntent/cancelIntent/constructWebhookEvent, impl réelle SDK + `setStripeGatewayForTests` pour les tests sans réseau/clé). Config `env.ts` : `PAYMENTS_ENABLED` (kill-switch), `STRIPE_SECRET_KEY` (manquant → 503), `STRIPE_WEBHOOK_SECRET`, `PLATFORM_COMMISSION_PCT` (défaut 15), `PAYMENT_AUTHORIZATION_TTL_DAYS` (7, max Stripe). Modèle `Payment` : +`authorizationExpiresAt` + `_id` ; `PaymentStatus` +`CANCELED` ; nouveau modèle `WebhookEvent` (idempotence, index unique `{provider,eventId}`). Service `payment.service.ts` :
    - **authorize** `POST /interventions/:id/payments/authorize` (owner-only 403, devis ACCEPTED requis sinon 409, 1 autorisation live max → 409) : crée un `PaymentIntent capture_method:manual` + `Payment` PENDING avec `amountCents`/`platformFeeCents` (env%)/`professionalAmountCents` (somme = amount, invariant modèle).
    - **capture** `capturePaymentByIntervention(interventionId)` (appelé best-effort au `complete` dans `intervention-actions.service.ts`) → `PAID` ; idempotent.
    - **cancel** `POST /payments/:paymentId/cancel` (owner, PENDING uniquement) → `CANCELED`.
    - **webhooks** `POST /webhooks/stripe` (monté `express.raw` AVANT le json parser global pour la vérification de signature sur octets bruts ; pas d'auth) : idempotence par table `WebhookEvent` (replay → `duplicate` no-op), gère `payment_intent.captured|canceled|payment_failed` → statuts `PAID|CANCELED|FAILED` + SSE `status-changed` (action=payment) ; type non-relevant → `ignored`.
  - **10 tests** (`payments.test.ts`, gateway fake injecté) : 401 / 403 non-owner / 409 sans devis ACCEPTED / 201 authorize avec split commission (21600 → fee 3240 à 15% + pro 18360) / 409 double autorisation / webhook captured→PAID + replay idempotent / canceled→CANCELED / 400 payload malformé / ignored invoice.created / capture-service→PAID → **142/142, 16 suites verts.**
  - **`7.2 Action `start` (QUOTE_ACCEPTED → IN_PROGRESS) — FAIT ✅`** : ajout de l'action pro `start` à `interventionActionEnum` (+ rebuild types dist) + `ACTION_TARGET.start = IN_PROGRESS` ; le graphe avait déjà l'arête `QUOTE_ACCEPTED → IN_PROGRESS`. Complète le cycle de capture : `DIAGNOSING →(quote)→ QUOTE_PENDING →(accept)→ QUOTE_ACCEPTED →(start)→ IN_PROGRESS →(complete)→ COMPLETED →(capture Stripe auto)→ PAID`. **2 tests actions** (start→IN_PROGRESS après devis accepté / 409 start avant devis) → **144/144, 16 suites verts.**
- **PHASE 08 (engagée)** : conversations/messages + notifications in-app.
  - **`8.0 Conversations, messages & notifications — FAIT ✅`** : modèles existants réutilisés (`Conversation` unique/intervention, `Message` content|media, `Notification`) + `_id` ajouté aux 3 interfaces Doc (même piège que Payment). Types Zod partagés `conversation-schemas.ts` (`sendMessageSchema` contenu 1–10000, query paginées, params). Services :
    - `conversation.service.ts` : garde participante via `resolveParticipants` (owner OU pro assigné, 403 sinon ; pro→user résolu car Messages/Notifications réfèrent des Users) ; `getOrCreateConversation` (création lazy + race sur index unique → re-read 11000) ; `sendMessage` (participant-only, maj `lastMessageAt`, **notification MESSAGE best-effort au destinataire**, SSE `status-changed action=message` sur le canal intervention) ; `listMessages` (oldest-first, cursor `before`) ; `listMyConversations` (union customer+pro dédupliquée).
    - `notification.service.ts` : `createNotification` (best-effort), `listNotifications` (filtre `unreadOnly`), `markNotificationRead` (owner-only 403).
  - **Routes** : `GET /interventions/:id/conversation` (get-or-create thread) ; `GET /conversations` ; `POST|GET /conversations/:conversationId/messages` (rate limit 60/min en envoi) ; `GET /conversations/notifications?unreadOnly=` ; `POST /conversations/notifications/:notificationId/read`.
  - **10 tests** (`conversations.test.ts`) : 401 thread / 403 stranger / get-or-create réutilise (unique) / 401 message / 403 non-participant send+read / envoi owner → lastMessageAt + notification pro + reply pro → notification owner + history oldest-first / 400 contenu vide / list mine customer+pro / 401 notifications / list+mark-read owner-only + unread vide → **154/154, 17 suites verts.**
  ...
  - **`6.1 Décision client sur devis — FAIT ✅`** : `POST /interventions/:id/quote/:quoteId/:decision` (`accept|reject`, param validé Zod `interventionQuoteParamSchema`) — service `decideQuote` : **owner-only** (403 pro/étranger, check `intervention.customer === userId`), état `QUOTE_PENDING` exigé (409 sinon), CAS atomique anti-double-décision → 409 ; **accept** → `QUOTE_ACCEPTED` + `Quote.status=ACCEPTED` (le pro pourra ensuite `QUOTE_ACCEPTED → IN_PROGRESS`, edge déjà au graphe) ; **reject** → retour `DIAGNOSING` (re-devis possible, boucle de transparence tarifaire F38350) + `Quote.status=REJECTED` ; `InterventionStatusHistory` + **SSE** `intervention.quote.accepted|rejected` (2 nouveaux types) + `status-changed`. **7 tests** (401 / 403 non-owner / 403 pro assigné / accept avec SSE + history / reject re-DIAGNOSING / 409 double décision / 400 verbe inconnu) → **128/128, 15 suites, typecheck + lint verts.**
  - **`6.2 GET /interventions/:id/quotes — FAIT ✅`** : liste des devis d'une intervention (newest first, avec line items fan-out `QuoteItem` par quote) — garde multi-tenant dans le service : **owner** (customer) OU **pro assigné** uniquement (403 pour un autre customer / pro non-assigné). `requireAuth` + `interventionIdParamSchema`. **4 tests** (401 / 403 stranger + pro non-assigné / 200 owner avec 2 devis [v1 REJECTED, v2 SENT après boucle reject → re-devis] + tri newest-first + totaux + items / 200 pro assigné) → **132/132, 15 suites verts.**
