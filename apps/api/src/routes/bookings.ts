...
router.get(
  "/slots",
  validate({ query: legacy.bookedSlotsQuerySchema }),
  listBookedSlots
);
...
router.get(
  "/mine",
  requireAuth,
  validate({ query: legacy.bookingListMineQuerySchema }),
  listMyBookings
);
...
router.post(
  ...
  validate({ body: legacy.createBookingBodySchema }),
  createBooking
);
...
router.patch(
  ...
  validate({ params: legacy.bookingIdParamSchema }),
  cancelBooking
);
